...
public class WindowBasedTopWord {
    ...
    public static void main(String[] args) {
        ...
        JavaReceiverInputDStream<String> searchLog = jssc.socketTextStream("localhost", 9999);
        // 将搜索日志转换成只有一个搜索词即可
        JavaDStream<String> searchWordDStream = searchLog.map(new Function<String, String>() {
            ...
            public String call(String searchLog) throws Exception {
                ...
                return searchLog.split(" ")[0];
            }

        });

        // 将搜索词映射为(searchWord, 1)的Tuple格式
        JavaPairDStream<String, Integer> searchWordPairDStream = searchWordDStream
        ...
        // 到这里就已经每隔10秒把之前60秒收集到的单词统计计数(Durations.seconds(5),每隔batch的时间间隔为5s,
        // 所以共有12个RDD),执行transform操作因为一个窗口60秒数据会变成一个RDD
        ...
        JavaPairDStream<String, Integer> finalDStream = searchWordCountsDStream
                .transformToPair(new Function<JavaPairRDD<String, Integer>, JavaPairRDD<String, Integer>>() {
                    ...
                    public JavaPairRDD<String, Integer> call(JavaPairRDD<String, Integer> searchWordCountsRDD)
                        ...
                        // 获取前三个word
                        List<Tuple2<String, Integer>> topSearchWordCounts = sortedSearchWordsRDD.take(5);
                        // 打印
